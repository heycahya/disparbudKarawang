...
const form = useForm({
  _method: 'PUT',
  name: props.culture.name,
  category: props.culture.category,
  description: props.culture.description,
  status: props.culture.status,
  cover_image: null,
  photos: null,
  photo_captions: null,
  deleted_photo_ids: []
...
const existingPhotos = ref([...(props.culture.photos || [])])
...
const submit = () => {
  ...
  form.post(route('admin.cultures.update', props.culture.id), {
    forceFormData: true
  })
}
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Edit Kebudayaan</h2>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-slate-100">
          <div class="p-6 text-slate-900">
            <form @submit.prevent="submit" class="space-y-6">
              <!-- Name -->
              <div>
                <label for="name" class="block text-sm font-semibold text-slate-700">Nama Kebudayaan / Seni / Tradisi</label>
                <input 
                  id="name"
                  v-model="form.name"
                  type="text" 
              ...
              <!-- Category -->
              <div>
                <label for="category" class="block text-sm font-semibold text-slate-700">Kategori Kebudayaan</label>
                <select 
                  id="category"
                ...
                >
                  ...
                  <option value="kesenian">Kesenian (Jaipong, Ajeng Sinden, Topeng Banjet, dll)</option>
                  <option value="tradisi">Tradisi</option>
                  <option value="warisan_budaya">Warisan Budaya</option>
                </select>
              ...
              <div>
                ...
                <div class="flex items-center gap-4 mt-1 mb-2" v-if="culture.cover_image">
                  <img :src="culture.cover_image" class="h-20 w-32 object-cover rounded-md border" alt="Cover Saat Ini" />
              ...
              <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                >
                  {{ form.processing ? 'Memperbarui...' : 'Perbarui Kebudayaan' }}
                </button>
              </div>
            </form>
          </div>
