...
watch(search, (value) => {
  router.get(route('admin.accommodations.index'), { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const deleteItem = (id) => {
  if (confirm('Apakah Anda yakin ingin menghapus akomodasi ini?')) {
    router.delete(route('admin.accommodations.destroy', id))
  }
}
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Manajemen Akomodasi & Hotel</h2>
    </template>
                  ...
                  <tr v-for="item in items.data" :key="item.id" class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div class="flex justify-end gap-2">
                        <Link :href="route('admin.accommodations.edit', item.id)" class="text-emerald-700 hover:text-emerald-900 transition">
                          Edit
                        </Link>
