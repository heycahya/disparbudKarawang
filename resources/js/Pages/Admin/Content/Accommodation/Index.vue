...
function applyFilters() {
    router.get(route('admin.accommodations.index'), {
        search: search.value,
...
function confirmDelete() {
    ...
    router.delete(route('admin.accommodations.destroy', itemToDelete.value.id), {
        onSuccess: () => {
...
<template>
    <Head title="Manajemen Akomodasi & Penginapan - Admin Disparbud" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold leading-tight text-slate-800 dark:text-slate-200">
                    Manajemen Akomodasi & Penginapan
                </h2>
                <Link
                ...
                <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div class="w-full md:w-72 relative">
                        <input
                            ...
                            type="text"
                            placeholder="Cari hotel/penginapan..."
                        ...
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 dark:bg-slate-950 text-slate-400 font-bold uppercase border-b border-slate-200/80 dark:border-slate-800">
                                <tr>
                                    ...
                                    <th class="px-6 py-4">Nama Akomodasi</th>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                <tr v-for="item in items.data" :key="item.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right space-x-2">
                                        <Link
                                            :href="route('admin.accommodations.edit', item.id)"
