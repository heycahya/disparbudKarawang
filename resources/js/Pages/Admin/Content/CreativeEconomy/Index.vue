...
function applyFilters() {
    router.get(route('admin.creative-economies.index'), {
        search: search.value,
...
function confirmDelete() {
    ...
    router.delete(route('admin.creative-economies.destroy', itemToDelete.value.id), {
        onSuccess: () => {
...
<template>
    <Head title="Manajemen Ekonomi Kreatif - Admin Disparbud" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold leading-tight text-slate-800 dark:text-slate-200">
                    Manajemen Ekonomi Kreatif (Ekraf)
                </h2>
                <Link
        ...
        <div class="py-8 bg-slate-50 min-h-[calc(100vh-8rem)] dark:bg-slate-950 font-sans">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
                        ...
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 dark:bg-slate-950 text-slate-400 font-bold uppercase border-b border-slate-200/80 dark:border-slate-800">
                                <tr>
                                    ...
                                    <th class="px-6 py-4">Nama Usaha / Produk</th>
                                    <th class="px-6 py-4">Subsektor</th>
                                    <th class="px-6 py-4">Alamat / Pemilik</th>
                                    <th class="px-6 py-4">Status</th>
                                    <th class="px-6 py-4 text-right">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                <tr v-for="item in items.data" :key="item.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right space-x-2">
                                        <Link
                                            :href="route('admin.creative-economies.edit', item.id)"
                                ...
                                <tr v-if="items.data.length === 0">
                                    <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                                        Belum ada data ekonomi kreatif.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <!-- Pagination -->
                    <div v-if="items.links && items.links.length > 3" class="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-center gap-1">
                        <Component
