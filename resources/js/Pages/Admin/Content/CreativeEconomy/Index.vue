...
watch(search, (value) => {
  router.get(route('admin.creative-economies.index'), { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const deleteItem = (id) => {
  if (confirm('Apakah Anda yakin ingin menghapus data pelaku ekraf ini?')) {
    router.delete(route('admin.creative-economies.destroy', id))
  }
}
</script>
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Manajemen Ekonomi Kreatif</h2>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-slate-100">
          <div class="p-6 text-slate-900">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
              ...
              <Link 
                :href="route('admin.creative-economies.create')" 
              ...
              <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50 rounded-md">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Usaha / Pelaku</th>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                  <tr v-for="item in items.data" :key="item.id" class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div class="flex justify-end gap-2">
                        <Link :href="route('admin.creative-economies.edit', item.id)" class="text-emerald-700 hover:text-emerald-900 transition">
                          Edit
                        </Link>
                  ...
                  <tr v-if="items.data.length === 0">
                    <td colspan="6" class="px-6 py-8 text-center text-sm text-slate-400">
                      Belum ada data pelaku ekonomi kreatif.
                    </td>
                  </tr>
                </tbody>
              </table>
