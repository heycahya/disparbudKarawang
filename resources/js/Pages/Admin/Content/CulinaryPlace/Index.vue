...
watch(search, (value) => {
  router.get(route('admin.culinary-places.index'), { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const deleteItem = (id) => {
  if (confirm('Apakah Anda yakin ingin menghapus tempat kuliner ini?')) {
    router.delete(route('admin.culinary-places.destroy', id))
  }
}
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Manajemen Kuliner Lokal</h2>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-slate-100">
          <div class="p-6 text-slate-900">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
              ...
              <Link 
                :href="route('admin.culinary-places.create')" 
              ...
              <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50 rounded-md">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Tempat</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Tipe</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Telepon</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Range Harga</th>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                  <tr v-for="item in items.data" :key="item.id" class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4">
                      ...
                      <div class="text-xs text-slate-400 line-clamp-1 max-w-[200px]">{{ item.address }}</div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div class="flex justify-end gap-2">
                        <Link :href="route('admin.culinary-places.edit', item.id)" class="text-emerald-700 hover:text-emerald-900 transition">
                          Edit
                        </Link>
                        <button @click="deleteItem(item.id)" class="text-rose-600 hover:text-rose-900 transition">
                          Hapus
                        </button>
                      </div>
                    </td>
