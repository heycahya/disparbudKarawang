...
watch(search, (value) => {
  router.get(route('admin.tourism-destinations.index'), { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const deleteDestination = (id) => {
  if (confirm('Apakah Anda yakin ingin menghapus destinasi wisata ini?')) {
    router.delete(route('admin.tourism-destinations.destroy', id))
  }
}
</script>
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Manajemen Destinasi Wisata</h2>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-tr-2xl rounded-bl-2xl border border-slate-100">
          <div class="p-6 text-slate-900">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
              ...
              <Link 
                :href="route('admin.tourism-destinations.create')" 
              ...
              <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50 rounded-tr-xl rounded-bl-xl">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Destinasi</th>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                  <tr v-for="item in destinations.data" :key="item.id" class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4 text-sm text-slate-500 line-clamp-1 max-w-[200px]">
                      {{ item.address }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span 
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div class="flex justify-end gap-2">
                        <Link :href="route('admin.tourism-destinations.edit', item.id)" class="text-emerald-700 hover:text-emerald-900 transition">
                          Edit
                        </Link>
                        <button @click="deleteDestination(item.id)" class="text-rose-600 hover:text-rose-900 transition">
                          Hapus
                        </button>
                      </div>
                    </td>
            ...
            <div class="flex justify-between items-center mt-6">
              <span class="text-xs text-slate-500">
                Menampilkan {{ destinations.from || 0 }} sampai {{ destinations.to || 0 }} dari {{ destinations.total }} data
              </span>
