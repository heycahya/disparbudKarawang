...
const applyFilters = () => {
  router.get(route('admin.service-rakyat.complaints.index'), { 
    search: search.value, 
    status: status.value 
  }, {
    preserveState: true,
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Inbox Laporan Pengaduan</h2>
    </template>
          ...
          <div class="p-6 text-slate-900">
              ...
              <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <div class="relative w-full sm:w-80">
                  <input 
                    v-model="search"
                    type="text" 
                    placeholder="Cari subjek atau pengirim..." 
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                  <tr v-for="item in complaints.data" :key="item.id" class="hover:bg-slate-50/50 transition">
                    <td class="px-6 py-4">
                      <div class="text-sm font-semibold text-slate-800 line-clamp-1">{{ item.subject }}</div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link :href="route('admin.service-rakyat.complaints.show', item.id)" class="text-emerald-700 hover:text-emerald-900 transition">
                        Detail & Review
                      </Link>
                    </td>
                  </tr>
            ...
            <div class="flex justify-between items-center mt-6">
              <span class="text-xs text-slate-500">
                Menampilkan {{ complaints.from || 0 }} sampai {{ complaints.to || 0 }} dari {{ complaints.total }} data
              </span>
