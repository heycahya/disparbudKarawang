...
const applyFilters = () => {
  router.get(route('admin.service-rakyat.event-broadcasts.index'), { 
    search: search.value, 
    status: status.value 
  }, {
    preserveState: true,
...
<template>
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-slate-800 leading-tight">Inbox Pengajuan Event Broadcasting</h2>
    </template>
          ...
          <div class="p-6 text-slate-900">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Event / Organisasi</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Pengaju</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Tanggal Event</th>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                  <tr v-for="item in eventBroadcasts.data" :key="item.id" class="hover:bg-slate-50/50 transition">
                    <td class="px-6 py-4">
                      <div class="text-sm font-semibold text-slate-800 line-clamp-1">{{ item.event_name }}</div>
                      <div class="text-xs text-slate-400 line-clamp-1">Oleh: {{ item.organization }}</div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link :href="route('admin.service-rakyat.event-broadcasts.show', item.id)" class="text-emerald-700 hover:text-emerald-900 transition">
                        Detail & Review
                      </Link>
                    </td>
                  </tr>
                  <tr v-if="eventBroadcasts.data.length === 0">
                    <td colspan="5" class="px-6 py-8 text-center text-sm text-slate-400">
                      Belum ada data pengajuan event.
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="flex justify-between items-center mt-6">
              <span class="text-xs text-slate-500">
                Menampilkan {{ eventBroadcasts.from || 0 }} sampai {{ eventBroadcasts.to || 0 }} dari {{ eventBroadcasts.total }} data
              </span>
              <div class="flex gap-1">
                <Link 
                  v-for="link in eventBroadcasts.links" 
                  :key="link.label"
                  :href="link.url || '#'" 
                  v-html="link.label"
                  :class="[
