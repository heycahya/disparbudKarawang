...
function confirmDelete() {
    ...
    router.delete(route('admin.galleries.destroy', itemToDelete.value.id), {
        onSuccess: () => {
        ...
        <div class="py-8 bg-slate-50 min-h-[calc(100vh-8rem)] dark:bg-slate-950 font-sans">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
                        ...
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 dark:bg-slate-950 text-slate-400 font-bold uppercase border-b border-slate-200/80 dark:border-slate-800">
                                <tr>
                                    ...
                                    <th class="px-6 py-4">Judul Dokumentasi</th>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                <tr v-for="item in galleries.data" :key="item.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <img :src="item.photo" class="h-12 w-20 object-cover rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm" alt="Photo" />
                                    </td>
                                    <td class="px-6 py-4 font-extrabold text-slate-900 dark:text-white text-sm">
                                        {{ item.title }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap font-medium text-slate-600 dark:text-slate-300 capitalize">
                                        <span class="px-2.5 py-1 inline-flex text-[10px] font-bold rounded-lg bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                                            {{ item.category }}
                                        </span>
                                    </td>
                    ...
                    <!-- Pagination -->
                    <div v-if="galleries.links && galleries.links.length > 3" class="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-center gap-1">
                        <Component
