...
<template>
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center space-x-6">
                        <Link :href="route('public.home')" class="text-xl font-bold text-[#0F5E3D] hover:opacity-90 transition">Disparbud Karawang</Link>
                        <Link :href="route('public.news.index')" class="text-sm font-medium hover:text-[#0F5E3D] transition">Berita</Link>
                        <Link :href="route('public.tourism.index')" class="text-sm font-medium text-[#0F5E3D] transition">Direktori</Link>
                    </div>
                </div>
            </div>
        </nav>
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <h1 class="text-3xl font-extrabold mb-2 text-gray-800 dark:text-gray-100">Direktori Pariwisata & Kebudayaan</h1>
            <p class="text-gray-600 dark:text-gray-400 mb-8 text-sm sm:text-base">Temukan pesona destinasi alam, warisan kebudayaan luhur, kuliner lezat, dan pelaku kreatif Kabupaten Karawang.</p>

            <!-- Tabs Section -->
            <div class="flex flex-wrap gap-2 border-b border-gray-200 dark:border-gray-700 pb-4 mb-8">
                <Link 
                    ...
                    :class="activeTab === 'tourism' 
                        ...
                        : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-tr-xl rounded-bl-xl'"
                >
                    Destinasi Wisata
                </Link>
                <Link 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                <div v-for="item in data.data" :key="item.id" class="bg-white dark:bg-gray-800 rounded-tr-2xl rounded-bl-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-lg transition duration-200">
                    <!-- Image -->
                    <img v-if="item.cover_image" :src="item.cover_image" alt="cover" class="w-full h-48 object-cover">
                    <div v-else class="w-full h-48 bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-400">
                        Tidak Ada Gambar
                    </div>
                    ...
                    <div class="p-6">
                        <!-- Badges/Metadata -->
                        <div class="mb-3">
                            <span v-if="activeTab === 'tourism' && item.category" class="text-xs font-semibold text-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 px-2.5 py-0.5 rounded-full">
                                {{ item.category.name }}
                            </span>
                            <span v-if="activeTab === 'culture'" class="text-xs font-semibold text-sky-700 bg-sky-50 dark:bg-sky-900/20 px-2.5 py-0.5 rounded-full">
                                {{ getCultureCategoryLabel(item.category) }}
                            </span>
                            <span v-if="activeTab === 'ekraf'" class="text-xs font-semibold text-purple-700 bg-purple-50 dark:bg-purple-900/20 px-2.5 py-0.5 rounded-full">
                                Ekonomi Kreatif
                            </span>
                            <span v-if="activeTab === 'accommodation'" class="text-xs font-semibold text-indigo-700 bg-indigo-50 dark:bg-indigo-900/20 px-2.5 py-0.5 rounded-full">
                                {{ getAccommodationTypeLabel(item.type) }}
                            </span>
                            <span v-if="activeTab === 'culinary'" class="text-xs font-semibold text-amber-700 bg-amber-50 dark:bg-amber-900/20 px-2.5 py-0.5 rounded-full">
            ...
            <!-- Empty State -->
            <div v-if="data.data.length === 0" class="bg-white dark:bg-gray-800 rounded-tr-2xl rounded-bl-2xl p-12 text-center border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <div class="flex justify-center space-x-2 mt-8">
                <div v-for="link in data.links" :key="link.label">
                    ...
                    <span 
                        v-else 
                        class="px-4 py-2 border rounded-lg text-sm text-gray-400 dark:text-gray-600 cursor-not-allowed bg-gray-100 dark:bg-gray-800" 
                        v-html="link.label"
                    />
                </div>
            </div>
